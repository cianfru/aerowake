...
def _build_rest_days_sleep(sleep_strategies: dict) -> List[RestDaySleepResponse]:
    """
    Extract rest-day / inter-duty gap sleep from sleep_strategies dict.

    Only includes entries keyed as ``rest_YYYY-MM-DD`` (generated by the
    inter-duty gap loop and the trailing OFF-day loop).  Duty-level sleep
    (keyed by duty_id) is already attached to each duty as ``sleep_quality``
    and rendered via ``duty.sleepEstimate`` on the frontend — including it
    here would produce duplicate sleep bars on the chronogram.
    """
    rest_days = []

    for key, data in sleep_strategies.items():
        if not key.startswith('rest_'):
            continue  # Skip duty-level keys (duty_id format)
