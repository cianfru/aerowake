 ...
 import { indexToKss, kssLabel, resolveKss } from '@/lib/risk-scale';
   ...
   return new Promise((resolve, reject) => {
     const events: EventAttributes[] = duties.map(duty => {
       ...
       const route = duty.flightSegments.map(seg => 
         `${seg.flightNumber}: ${seg.departure} → ${seg.arrival} (${seg.departureTime}-${seg.arrivalTime})`
       ).join('\n');
       ...
       return {
         ...
         title: `✈️ ${duty.flightSegments.map(s => s.flightNumber).join(' • ') || 'Flight Duty'}`,
         description: [
           `${riskEmoji} Risk Level: ${duty.overallRisk}`,
           `Predicted sleepiness: peak KSS ${(resolveKss(duty.maxKss, duty.minPerformance) ?? 0).toFixed(1)} (${kssLabel(resolveKss(duty.maxKss, duty.minPerformance) ?? 5)}), avg KSS ${indexToKss(duty.avgPerformance).toFixed(1)}`,
 ...
 export function generateGoogleCalendarUrl(duty: DutyAnalysis): string {
   ...
   const title = `✈️ ${duty.flightSegments.map(s => s.flightNumber).join(' • ') || 'Flight Duty'}`;
   ...
   const details = [
     ...
     `Predicted sleepiness: peak KSS ${(resolveKss(duty.maxKss, duty.minPerformance) ?? 0).toFixed(1)}`,
     `Route: ${route}`,
     'Generated by Aerowake'
   ].join('\n');
 ...
 export function openGoogleCalendarBatch(duties: DutyAnalysis[]): void {
   ...
   alert(
     ...
     `3. Click Settings (gear icon) → Import & export\n` +
