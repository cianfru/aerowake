import { AlertTriangle, Plane, Clock, Moon, ChevronDown, Globe, Zap, Users, Monitor, BookOpen, PlaneTakeoff } from 'lucide-react';
...
export function DutyDetails({ duty, dutyCrewOverride, onCrewChange, onCrewReset }: DutyDetailsProps) {
  ...
  // Determine effective crew set: override takes priority, else parser auto-detected
  const autoDetectedCrewSet = duty.ulrCrewSet || 'crew_b';
  ...
  return (
    <div className="space-y-4 md:space-y-6 animate-fade-in">
          ...
          <CardTitle className="text-base md:text-lg flex items-center gap-2">
            ...
            {isTrainingDuty(duty) && (
              <Badge variant="info" className="text-[10px] flex items-center gap-1">
                {duty.dutyType === 'simulator' ? <Monitor className="h-3 w-3" /> : <BookOpen className="h-3 w-3" />}
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-3 md:gap-4 sm:grid-cols-3">
            ...
            <div className="space-y-0.5 md:space-y-1">
              ...
              <p className="font-medium text-sm md:text-base">{isTrainingDuty(duty) ? '—' : duty.sectors}</p>
            </div>
            <div className="space-y-0.5 md:space-y-1">
      ...
      {/* Section 2: Performance Summary — key insights at the top */}
          ...
          <CardHeader className="pb-2 md:pb-3">
            <CardTitle className="flex items-center gap-2 text-sm md:text-base">
              {duty.dutyType === 'simulator'
                ? <Monitor className="h-3.5 w-3.5 md:h-4 md:w-4" style={{ color: 'hsl(var(--simulator))' }} />
                : <BookOpen className="h-3.5 w-3.5 md:h-4 md:w-4" style={{ color: 'hsl(var(--ground-training))' }} />}
              Training Session
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="rounded-xl border border-border/50 bg-secondary/30 p-3 md:p-4 space-y-2.5">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs md:text-sm">
                ...
                <span>{duty.reportTimeLocal}</span>
                <span className="text-muted-foreground">Release</span>
                <span>{duty.releaseTimeLocal}</span>
                <span className="text-muted-foreground">Duration</span>
                <span>{(duty.dutyHours ?? 0).toFixed(1)}h</span>
              </div>
              {duty.trainingAnnotations && duty.trainingAnnotations.length > 0 && (
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className="text-[10px] md:text-xs text-muted-foreground">Annotations:</span>
                  {duty.trainingAnnotations.map(a => (
                    <Badge key={a} variant="outline" className="text-[9px] md:text-[10px]">{a}</Badge>
                  ))}
                </div>
              )}
            </div>
      ...
      {/* Crew Assignment — compact A/B toggle for augmented/ULR duties */}
                  ...
                  {onCrewReset && (
                    <button
                      onClick={() => {
                        ...
                        toast.info(`Reset to auto-detected: Crew ${autoDetectedCrewSet === 'crew_a' ? 'A' : 'B'}`);
                      }}
